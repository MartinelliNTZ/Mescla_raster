# -*- coding: utf-8 -*-
"""
MOSAICO MERGE + DIAGNÓSTICO (ECW) v2
=====================================
v2: Adicionado timestamp [HH:MM:SS] em todas as mensagens para acompanhamento.
...
Para ECW, usa o gdal_translate do QGIS com o plugin ECW.
"""
...
# ╔══════════════════════════════════════════════════════════════╗
# ║         TIMESTAMP — TODOS OS PRINTS TERÃO HORÁRIO          ║
# ╚══════════════════════════════════════════════════════════════╝

def ts():
    """Retorna o timestamp atual no formato [HH:MM:SS]."""
    return datetime.now().strftime("[%H:%M:%S]")


def log(msg):
    """Print com timestamp no início."""
    print(f"{ts()} {msg}")
...
OUTPUT_DIR = r"G:\BR070\ETAPA2\OUTPUT\ORTOMOSAICO_FINAL_01\ecw\2"
...
def setup_gdal_environment():
    """Configura GDAL_DRIVER_PATH para usar plugin ECW do QGIS."""
            ...
            if has_ecw:
                ...
                try:
                    ...
                    if result.returncode == 0:
                        log(f"GDAL encontrado: {qver['path']}")
                        log(f"Plugin ECW em: {plugin_dir}")
                        return gdal_bin, env
                except Exception:
                    pass
    return None, None
...
def convert_ecw_to_tiff(ecw_path, tiff_path, gdal_cmd, env):
    ...
    log(f"[CONV] Comando: {' '.join(cmd)}")
    log(f"[CONV] Iniciando conversao... (pode levar varios minutos para arquivos grandes)")
    
    t_start = time.time()
    ok, err = run_gdal(cmd, env, timeout=14400)
    t_elapsed = time.time() - t_start
    
    if ok:
        size_mb = os.path.getsize(tiff_path) / (1024 * 1024)
        log(f"[CONV] OK! ({t_elapsed:.0f}s) Tamanho: {size_mb:.1f} MB")
        return True
    else:
...
def convert_tiff_to_ecw(tiff_path, ecw_path, gdal_cmd, env):
    ...
    log(f"[CONV] Iniciando conversao... (pode levar varios minutos)")
...
def ensure_4band_rgb(ds):
    ...
    if count == 4:
        ...
        try:
            ...
            return rgb, np.ones_like(data[0], dtype=np.float32) * dtype_max
    else:
...
def run_mosaic(input_files, output_path):
    ...
    log("=" * 60)
    log(f"MOSAICO FEATHERING")
    log(f"Saida em: {'ECW' if is_ecw_output else 'GeoTIFF'}")
    log("=" * 60)
    ...
    log(f"[MOSAICO] Datasets abertos ({time.time()-t0_dataset:.1f}s)")

    # Grid de referencia
    log("[MOSAICO] Criando grid de referencia...")
    ref_ds = datasets[0]
    ...
    try:
        ...
        del mosaic_ref
        log(f"[MOSAICO] Grid criado ({time.time()-t0_merge:.1f}s): {width}x{height} dtype={out_dtype}")
    except Exception as e:
    ...
    # Acumuladores
    log("[MOSAICO] Inicializando acumuladores...")
    ...
    for i, ds in enumerate(datasets):
        ...
        log(f"[MOSAICO] Processando {i+1}/{len(datasets)}: {name}")
        ...
        log(f"[MOSAICO]   Distance transform...")
        dist = distance_transform_edt(mask).astype(np.float32)
        dmax = dist.max()
        if dmax > 0: dist /= dmax
        del mask
        
        log(f"[MOSAICO]   Reprojetando peso...")
        ...
        log(f"[MOSAICO]   Reprojetando RGB...")
        ...
        log(f"[MOSAICO]   Reprojetando Alpha...")
        ...
        if peso.max() == 0:
            log(f"[MOSAICO]   Peso zerado, usando alpha...")
            peso = dst_alpha / dtype_max
            if peso.max() == 0:
                log(f"[MOSAICO]   Alpha zerado, peso uniforme...")
                peso = np.ones_like(peso, dtype=np.float32) * 0.001
        
        log(f"[MOSAICO]   Acumulando...")
        ...
        log(f"[MOSAICO]   OK ({elapsed:.1f}s)")

    # Normalizar
    log(f"[MOSAICO] Normalizando...")
    t0_norm = time.time()
    valido = accum_w > 0
    ...
    log(f"[MOSAICO] Normalizado ({time.time()-t0_norm:.1f}s)")

    # Salvar TIFF intermediario
    tiff_intermediario = output_path
    if is_ecw_output:
        tiff_intermediario = output_path.replace('.ecw', '_temp.tif')

    log(f"[MOSAICO] Salvando TIFF intermediario...")
    ...
    del resultado
    log(f"[MOSAICO] TIFF salvo ({time.time()-t0_save:.1f}s)")
    ...
    if is_ecw_output:
        ...
        if gdal_cmd:
            tiff_size = os.path.getsize(tiff_intermediario) / (1024 * 1024 * 1024)
            log(f"[MOSAICO] Convertendo TIFF ({tiff_size:.1f} GB) para ECW...")
            if convert_tiff_to_ecw(tiff_intermediario, output_path, gdal_cmd, env):
                ...
                log(f"[MOSAICO] Falha ECW. Mantendo TIFF como fallback.")
                fallback = output_path.replace('.ecw', '_fallback.tif')
                shutil.move(tiff_intermediario, fallback)
                output_path = fallback
        else:
            log(f"[MOSAICO] Plugin ECW nao disponivel. Mantendo TIFF.")
            fallback = output_path.replace('.ecw', '_fallback.tif')
            shutil.move(tiff_intermediario, fallback)
            output_path = fallback
...
def main():
    ...
    log("=" * 70)
    log(" MOSAICO MERGE + DIAGNOSTICO (ECW) v2")
    log("=" * 70)
    log(f"")
    ...
    if gdal_cmd:
        log("Plugin ECW configurado!")
    else:
        log("ATENCAO: Plugin ECW nao encontrado no QGIS")
        log("Tente copiar gdal_ECW_JP2ECW.dll para pasta do script")
    ...
    if len(input_files) == 0:
        ...
        log(f"Procurando: {', '.join(INPUT_EXTENSIONS)}")
        sys.exit(1)

    log(f"")
    log(f"Arquivos encontrados: {len(input_files)}")
    ...
    log(f"")
    log("=" * 60)
    log("PRE-PROCESSAMENTO: Conversao ECW -> TIFF")
    log("=" * 60)
        ...
        if f.lower().endswith('.ecw'):
            if gdal_cmd:
                ...
                if convert_ecw_to_tiff(f, tiff_temp, gdal_cmd, gdal_env):
                    ...
                    log(f"[PRE] OK: {os.path.basename(f)} convertido")
                else:
                    log(f"[PRE] ERRO: Pulando {os.path.basename(f)}")
            else:
                log(f"[PRE] ERRO: Nao pode ler {os.path.basename(f)} (sem plugin ECW)")
        else:
    ...
    log(f"")
    log(f"Arquivos prontos para mosaico: {len(arquivos_processados)}")

    # Diagnostico dos inputs
    if RUN_DIAGNOSTIC_INPUTS:
        log(f"")
        log("=" * 70)
        log(" DIAGNOSTICO DOS ARQUIVOS DE ENTRADA")
        log("=" * 70)
        for f in input_files:
            base = os.path.splitext(os.path.basename(f))[0]
            json_path = os.path.join(OUTPUT_DIR, f"diagnostico_input_{base}.json")
            log(f"")
            log(f"Processando diagnostico: {os.path.basename(f)}")
    ...
    if RUN_MOSAIC:
        ...
        if not os.path.exists(output_path):
            ...
            shutil.rmtree(temp_dir, ignore_errors=True)
            sys.exit(1)
        log(f"Pulando execucao do mosaico (RUN_MOSAIC=False)")
    ...
    if RUN_DIAGNOSTIC_OUTPUT and os.path.exists(output_path):
        ...
        log(f"")
        log(f"Processando diagnostico: {os.path.basename(output_path)}")
        report = diagnostic_report(output_path)
        save_diagnostic_json(report, json_out)

        log(f"")
        log(f"  {'='*50}")
        log(f"  RESUMO DO MOSAICO FINAL")
        log(f"  {'='*50}")
    ...
    log(f"")
    log("=" * 70)
    log(" ARQUIVOS GERADOS NA PASTA OUTPUT")
    log("=" * 70)
    ...
    if not KEEP_TEMP_TIFFS:
        shutil.rmtree(temp_dir, ignore_errors=True)
        log(f"Diretorio temporario removido: {temp_dir}")

    t_total = time.time() - t_inicio
    log(f"")
    log("=" * 70)
    log(f" PROCESSO CONCLUIDO COM SUCESSO! (tempo total: {t_total/60:.1f} min)")
    log("=" * 70)
